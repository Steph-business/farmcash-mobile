...
/// Page Retirer Wallet — montant à retirer, destinataire (OM), code PIN
/// MoMo (visuel), CTA sticky « Confirmer le retrait ».
///
/// Note service : il n'y a pas d'endpoint « withdraw » exposé dans
/// `FinanceService` (uniquement `payout` qui nécessite un moyenPayementId
/// préalablement enregistré). On reste donc côté UI : à la confirmation,
/// on affiche un snackbar « Demande de retrait envoyée » et on pop.
class WalletRetirerPage extends ConsumerStatefulWidget {
  const WalletRetirerPage({super.key});

  @override
  ConsumerState<WalletRetirerPage> createState() => _WalletRetirerPageState();
}

class _WalletRetirerPageState extends ConsumerState<WalletRetirerPage> {
...
class _BalanceCard extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final formatted = NumberFormat('#,##0', 'fr_FR').format(245800);
    return Container(
  ...
  Widget build(BuildContext context) {
    return Wrap(
      ...
      children: [
        ...
        // « Tout » → bordure verte permanente (cf. spec) + active visuel
        // si on l'a tappé.
        _QChip(
          label: 'Tout',
          highlighted: true,
...
class _QChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final borderColor = (highlighted || active) ? AppColors.primary : AppColors.border;
...
class _PinDots extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    // 2 remplis (●), 2 vides (point gris central) — strictement la maquette.
    return Padding(
      padding: const EdgeInsets.symmetric(vertical: 14),
      child: Row(
