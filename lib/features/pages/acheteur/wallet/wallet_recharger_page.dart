...
/// Catalogue des méthodes affichées à l'acheteur — numéro OM
/// 07 11 22 33 44, Wave renseigné via 07 88 99 11 22 (compte connu mais
/// non encore sélectionné comme méthode par défaut).
final _methodes = catalogueMethodes(
  numeroOmLie: '07 11 22 33 44',
  waveSousTitre: '07 88 99 11 22',
);

const _kQuickAmounts = [5000, 10000, 50000, 100000];

/// Page Recharger Wallet acheteur — montant + méthode de paiement + CTA
/// sticky.
///
/// Lecture du solde via `financeService.getWallet()`. Au tap "Recharger",
/// appelle `financeService.topupWallet(...)` avec une clé d'idempotence
/// pseudo-aléatoire.
class WalletRechargerAcheteurPage extends ConsumerStatefulWidget {
  const WalletRechargerAcheteurPage({super.key});

  @override
  ConsumerState<WalletRechargerAcheteurPage> createState() =>
      _WalletRechargerAcheteurPageState();
}

class _WalletRechargerAcheteurPageState
    extends ConsumerState<WalletRechargerAcheteurPage> {
  late final TextEditingController _amountCtrl;
  int _selectedQuick = 50000;
